            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="card-tools">
                  <div class="input-group input-group-sm">
                    <button type="button" class="btn btn-block btn-primary btn-sm" @click="collectionDetailStore.saveCollection()"><i class="fas fa-plus"></i> Thêm mới</button>
                  </div>
                </div>
                ...
                <div class="row">
                  <div class="col-sm-9">
                    <div class="form-group">
                      ...
                      <input type="text" @input="handleInputKeySearch" @keyup.enter="collectionListStore.getListPager" v-model="collectionListStore.filters.keySearch" class="form-control" id="name" placeholder="Gõ từ khóa và enter">
                    </div>
                  </div>
                  <div class="col-sm-2" :class="tab == '#trash' ? 'd-none' : ''">
                    <div class="form-group">
                      ...
                      <Treeselect
                        ...
                        :clearable="false"
                        v-model="collectionListStore.filters.status"
                        :options="statusList" />
                    </div>
                  </div>
                  <div class="form-group" title="Reset bộ lọc" @click="collectionListStore.getListPager(1, 'reset')">
                    <label>&nbsp;</label>
                ...
                <table class="table table-hover text-nowrap" v-if="tables.rows.length">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in tables.rows" :key="index">
                      ...
                      <td>
                        <router-link :to="'/app/collection/'+item.id" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Sửa</router-link>
                      </td>
                    </tr>
                  </tbody>
                </table>
                <LabelText v-else-if="collectionListStore.loading" :classAlert="'text-center alert-dismissible'" :classInput="'blink'" text="Đang tải danh mục ..." />
                <LabelText v-else text="Không tìm thấy bộ sưu tập nào" :classAlert="'text-center alert-dismissible'" />
              </div>
              <!-- /.card-body -->
              <div class="card-footer clearfix" v-if="tables.rows.length">
                <Paginate
                  ...
                  :margin-pages="2"
                  :click-handler="collectionListStore.getListPager"
                  :prev-text="'Prev'"
                  :next-text="'Next'"
...
import { useCollectionListStore, useCollectionDetailStore } from '../../stores/modules/collectionStore'

export default defineComponent({
  props: {},
  data() {
    return {
      collectionListStore: new useCollectionListStore(),
  ...
  computed: {
    tables() {
      return this.collectionListStore.dataTable;
    },
    tab() {
      let value = this.$route.hash || ''
      this.collectionListStore.filters.tab = value
      return value
    }
  },
  methods: {
    handleInputKeySearch(e) {
      if (!e.target.value) {
        this.collectionListStore.getListPager()
      }
    },
    changeOptionsFiltersStatus(node) {
      this.collectionListStore.filters.status = node.id
      this.collectionListStore.getListPager()
    },
    ...
    changeStatus(status) {
      this.collectionListStore.changeStatus(this.idsChecked, status)
      this.idsChecked = []
    },
    changeStorage(tab) {
      window.location.href = `/app/collection${tab}`
      this.collectionListStore.filters.tab = tab
      this.collectionListStore.getListPager(1, true)
    }
  },
  created() {
    this.collectionListStore.getListPager()
